...
class AppDaoTest {
    ...
    /*
     * 테스트용 DB와 DAO를 주입받는다. private로 선언 x
    ...
    // 예시 테스트: 월별 통계 쿼리 테스트
    @Test
    fun getMonthlyStats_calculatesCorrectly() =
        runBlocking {
            // Arrange (준비)
    ...
    @Test
    fun getOngoingSessionId_returnsNull_whenAllSessionsAreFinished() =
        runBlocking {
    ...
    @Test
    fun getOngoingSessionId_returnsLatestSessionId_whenMultipleOngoingSessionsExist() =
        runBlocking {
    ...
    @Test
    fun getAllSessions_emitsNewList_whenSessionIsAdded() =
        runBlocking {
            // given: 초기에 세션이 하나 있음

...
class DrivingRepositoryImpl
    ...
    constructor(
         ...
         */
        override fun getOngoingSessionId(): Long? = dao.getOngoingSessionId()
         ...
         */
        override fun getAllSessions(): Flow<List<DrivingSession>> = dao.getAllSessions()
         ...
         */
        override fun getSessionData(sessionId: Long): Flow<List<DrivingDataPoint>> = dao.getDataPoints(sessionId)
         ...
         */
        override suspend fun saveDataPoint(point: DrivingDataPoint) =
            withContext(Dispatchers.IO) {
         ...
         */
        override suspend fun syncSession(sessionId: Long) =
            withContext(Dispatchers.IO) {
        /*
        // 1. 세션 및 데이터 포인트 조회
        val session = dao.getSessionById(sessionId)
         ...
         */
        override suspend fun insertSession(session: DrivingSession) =
            withContext(Dispatchers.IO) {
                dao.insertSession(session)
            }
        ...
        override fun getSessionsInRange(
            startMillis: Long,
            endMillis: Long,
        ): Flow<List<DrivingSession>> = dao.getSessionsInRange(startMillis, endMillis)
         ...
         */
        override suspend fun getSessionSummaryOnce(sessionId: Long): DrivingSessionSummary? = dao.getSessionSummaryOnce(sessionId)
         ...
         */
        override fun getSessionSummaryFlow(sessionId: Long): Flow<DrivingSessionSummary?> = dao.getSessionSummaryFlow(sessionId)
        ...
        override fun getSessionSummariesInRange(
            startMillis: Long,
            endMillis: Long,
        ): Flow<MonthlyStats> = dao.getMonthlyStats(startMillis, endMillis)
    }
